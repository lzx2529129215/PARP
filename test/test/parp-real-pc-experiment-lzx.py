...
def load_config(path: Path) -> dict[str, Any]:
    ...
    if "cold_writeback_gate_hot_reuse" in value["scenarios"]:
        ...
        if str(value.get("pressure_mode", "")) != "cgroup_writeback_gate":
            raise ValueError("writeback-gate scenario requires cgroup_writeback_gate pressure")
    return value
...
def scope_snapshot(path: Path | None) -> dict[str, Any]:
    ...
    return {
        ...
        "io_stat": ACCEPT.read_io_stat(path / "io.stat"),  # lzx-note
...
def prepare_assets(config: dict[str, Any], run_dir: Path) -> dict[str, Any]:
    ...
    for name in (
        "local-page.html", "writer-test.txt", "mail-test.eml",
        "image-test.png", "audio-test.wav", "document-test.pdf",
    ):
...
def generate_scenario(
    ...
    writeback_gate = scenario_name == "cold_writeback_gate_hot_reuse"  # lzx-note
    ...
    if scenario_name == "predicted_return":
    ...
    return {
        ...
        "metadata": {
            ...
            "pressure_kind": (
                "global_memfree_reclaim_probe" if global_pressure
...
def parse_action_latencies(path: Path) -> dict[str, Any]:
    try:
    ...
    for row in rows:
        ...
        if row.get("phase") == "start":
            ...
            samples.append({
                ...
                "duration_ms": (timestamp - starts[key]) / 1_000_000.0,
            })
    return {
        "samples": len(samples),
        "actions": samples,
...
def run_one(
    ...
    if policy == "bin_workload_lstm":
        ...
        if (
            scenario_name == "cold_writeback_gate_hot_reuse"
    ...
    result = {
        ...
        "post_reuse": reuse,
        "interaction_latency": parse_action_latencies(run_dir / "automation-trace.csv"),
        ...
        } if scenario_name == "cold_writeback_gate_hot_reuse" else {},  # lzx-note
