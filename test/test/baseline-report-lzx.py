...
def sha256_file(path: Path) -> str | None:
    """Return an evidence hash without loading large trace files into memory."""  # lzx-note
    if not path.is_file():
        return None
...
def suite_metrics(summary_path: Path) -> dict[str, Any]:
    ...
    attempts: list[dict[str, Any]] = []  # lzx-note: invalid rounds remain diagnostic evidence
    for index, item in enumerate(all_results):
        ...
        attempts.append({
            ...
            "case_start": automation.get("case_start"),
            "case_done": automation.get("case_done"),
            "expected_steps": item.get("expected_steps"),
            "trace_page_fault_user": trace.get("page_fault_user"),
            "trace_loss_total": trace.get("loss_total"),
            "cgroup_pgfault": cgroup.get("pgfault_delta"),
            "cgroup_pgmajfault": cgroup.get("pgmajfault_delta"),
            "workingset_refault_file": cgroup.get("workingset_refault_file_delta"),
            "workingset_refault_anon": cgroup.get("workingset_refault_anon_delta"),
            "pgscan": cgroup.get("pgscan_delta"),
            "pgsteal": cgroup.get("pgsteal_delta"),
            "pswpin": cgroup.get("pswpin_delta"),
            "pswpout": cgroup.get("pswpout_delta"),
            "memory_high_events": cgroup.get("events_high_delta"),
            "cgroup_oom": cgroup.get("oom_delta"),
            "cgroup_oom_kill": cgroup.get("oom_kill_delta"),
            "host_oom_kill": item.get("system", {}).get("host_oom_kill_delta"),
            "failure_total": events.get("failure_total"),
            "launch_ready_latency_mean_ms": item.get("launch", {}).get("mean_ms"),
            "launch_ready_latency_p95_ms": item.get("launch", {}).get("p95_ms"),
            "monitor_extrema": monitor_extrema(suite_root, [index]),
        })
    scenario_plans: list[dict[str, Any]] = []  # lzx-note: exact paired-replay contract
    for index in range(len(all_results)):
        plan_path = suite_root / f"round-{index + 1:02d}/scenario-plan.json"
    ...
    return {
        ...
        "case_done_attempted_total": sum(int(item.get("automation", {}).get("case_done", 0)) for item in all_results),  # lzx-note
        "attempts": attempts,  # lzx-note
        "workload_contract": summary.get("workload_contract", {}),
        "metrics": metrics,
        "monitor_extrema": monitor_extrema(suite_root, valid_indices),  #lzx
        "attempt_monitor_extrema": monitor_extrema(suite_root),  # lzx-note
        "summary_sha256": sha256_file(summary_path),  # lzx-note
        "scenario_plans": scenario_plans,  # lzx-note
        "preflight": preflight,
    }
...
def build_report(hotcold_path: Path, peak_path: Path) -> tuple[dict[str, Any], str]:
    ...
    if hot_baseline is None:
        raise RuntimeError("hotcold 没有有效轮，无法建立 PageFault 基线")
    pagefault_target_20 = hot_baseline * 0.80
    pagefault_target_30 = hot_baseline * 0.70
    peak_target_30 = peak_baseline * 0.70 if peak_baseline is not None and peak_baseline > 0 else None
    peak_verdict = (
        "NO_VALID_ROUNDS_PSI_ABORT"
        if peak_baseline is None
        else "ZERO_BASELINE_REQUIRES_CALIBRATION"
        if peak_baseline == 0
        else "BASELINE_ONLY_WAITING_FOR_APPLY_PAIR"
    )
    ...
    peak_target_text = (
        "N/A（无有效peak轮）"
        if peak_baseline is None
        else "N/A（基线为0，必须先校准出非零基线）"
        if peak_target_30 is None
        else number(peak_target_30)
    )
    peak_conclusion = (
        "三轮均触发安全停止，无有效基线"
        if peak_baseline is None
    ...
    hot_extrema = hotcold["monitor_extrema"]
    peak_extrema = peak["monitor_extrema"] if peak["rounds_valid"] else peak["attempt_monitor_extrema"]
    lines = [
        ...
        f"| 峰值异常总数 | `{number(peak_baseline)}` 次/轮 | — | `{peak_target_text}` | {peak_conclusion} |", "",
    ...
    invalid_peak = [item for item in peak["attempts"] if item["status"] != VALID_STATUS]  # lzx-note
    if invalid_peak:
        lines += [
            "## 峰值实验：未完成轮次的压力诊断证据", "",
        ...
        for item in invalid_peak:
            extrema = item["monitor_extrema"]
            lines.append(
                f"| {item['round']} | {number(item['case_done'])}/{number(item['expected_steps'])} | `{item['abort_reason'] or item['status']}` | "
                f"{gib(extrema['min_memavailable_bytes'])} | {gib(extrema['min_swapfree_bytes'])} | "
                f"{extrema['max_psi_some_avg10']:.3f}/{extrema['max_psi_full_avg10']:.3f} | "
                f"{number(item['memory_high_events'])} | {number(item['cgroup_oom'])}/{number(item['cgroup_oom_kill'])}/{number(item['host_oom_kill'])} | "
                f"{number(item['trace_page_fault_user'])}/{number(item['cgroup_pgfault'])} | "
                f"{number(item['pgscan'])}/{number(item['pgsteal'])} | {number(item['trace_loss_total'])} |"
            )
        lines += [
            "",
            f"- 三轮合计完成步骤：`{peak['case_done_attempted_total']}` / `{sum(int(item.get('expected_steps') or 0) for item in peak['attempts'])}`。",
    ...
    lines += [
        ...
        "|---|---:|---:|---:|---:|",
        f"| 冷热 | `{gib(hot_extrema['min_memavailable_bytes'])}` | `{gib(hot_extrema['min_swapfree_bytes'])}` | `{hot_extrema['max_psi_some_avg10']:.3f}` / `{hot_extrema['max_psi_full_avg10']:.3f}` | `{gib(hot_extrema['max_test_cgroup_memory_current_bytes'])}` |",
        f"| 峰值（含安全停止轮） | `{gib(peak_extrema['min_memavailable_bytes'])}` | `{gib(peak_extrema['min_swapfree_bytes'])}` | `{peak_extrema['max_psi_some_avg10']:.3f}` / `{peak_extrema['max_psi_full_avg10']:.3f}` | `{gib(peak_extrema['max_test_cgroup_memory_current_bytes'])}` |", "",
        "## Swap、限流与OOM累计变化", "",
        "| 实验 | pswpin | pswpout | memory.high | memory.max | cgroup OOM/OOM kill | 宿主OOM kill | 弹窗最大数 |",
        "|---|---:|---:|---:|---:|---:|---:|---:|",
        f"| 冷热 | `{number(hot_extrema['pswpin_delta'])}` | `{number(hot_extrema['pswpout_delta'])}` | `{number(hot_extrema['test_cgroup_memory_high_delta'])}` | `{number(hot_extrema['test_cgroup_memory_max_delta'])}` | `{number(hot_extrema['test_cgroup_oom_delta'])}` / `{number(hot_extrema['test_cgroup_oom_kill_delta'])}` | `{number(hot_extrema['host_oom_kill_delta'])}` | `{number(hot_extrema['max_low_memory_popup_count'])}` |",
        f"| 峰值（含安全停止轮） | `{number(peak_extrema['pswpin_delta'])}` | `{number(peak_extrema['pswpout_delta'])}` | `{number(peak_extrema['test_cgroup_memory_high_delta'])}` | `{number(peak_extrema['test_cgroup_memory_max_delta'])}` | `{number(peak_extrema['test_cgroup_oom_delta'])}` / `{number(peak_extrema['test_cgroup_oom_kill_delta'])}` | `{number(peak_extrema['host_oom_kill_delta'])}` | `{number(peak_extrema['max_low_memory_popup_count'])}` |", "",
        "## 判读说明", "",
        ...
        f"- 当前为observe运行，effective-tier mode=`{env['effective_tier_mode']}`、apply_compiled=`{env['apply_compiled']}`；未开启Apply，不能单独给出优化改善率。",  # lzx-note
        "- 峰值轮若因安全阈值停止，其部分指标只能用于诊断，不能与有效轮混算；配对实验必须在两侧复现相同停止状态或先固定场景安全边界。",  # lzx-note
        "- 峰值异常基线为0或没有有效轮时均没有可用改善率分母，需要先得到可配对的有效峰值基线。", "",  # lzx-note
        "## Linux 配对重放证据", "",  # lzx-note
        f"- hotcold summary SHA-256：`{hotcold['summary_sha256'] or 'N/A'}`",
        f"- peak summary SHA-256：`{peak['summary_sha256'] or 'N/A'}`", "",
        "| 场景 | 轮次 | seed | scenario-plan SHA-256 |",  # lzx-note
        "|---|---:|---:|---|",
    ]
    for suite_name, suite_data in (("hotcold", hotcold), ("peak", peak)):
        for plan in suite_data["scenario_plans"]:
            lines.append(
                f"| {suite_name} | {plan['round']} | {number(plan['seed'])} | `{plan['sha256'] or 'N/A'}` |"
            )
    lines += [
        "",
        "> Linux 对照轮必须通过本报告原始结果目录执行 `--replay-from`，不能只复用seed后重新随机生成。peak对照还应保留相同安全阈值；若同样停止，则比较完成步数、停止时资源状态和部分诊断指标。", "",  # lzx-note
        "## 原始数据", "",
