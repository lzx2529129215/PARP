...
class SemanticAutomationTests(unittest.TestCase):
    def test_login_free_app_window_mapping(self) -> None:
        # lzx-note: These mappings are the contract used by switch/foreground verification.
        cases = [
            ("Calendar", "gnome-calendar|Gnome-calendar", "gnome-calendar", "CALENDAR"),
            ("Rhythmbox", "rhythmbox|Rhythmbox", "rhythmbox", "RHYTHMBOX"),
            ("automation-image.png — Image Viewer", "eog|Eog", "eog", "IMAGE_VIEWER"),
            ("automation-image.png - Shotwell", "shotwell|Shotwell", "shotwell", "SHOTWELL"),
            ("System Monitor", "gnome-system-monitor|Gnome-system-monitor", "gnome-system-mo", "SYSTEM_MONITOR"),
            ("Klondike", "sol|Sol", "sol", "SOLITAIRE"),
        ]
        for title, wm_class, comm, expected in cases:
            with self.subTest(app=expected):
                self.assertEqual(map_window_app(title, wm_class, comm), expected)
                self.assertEqual(infer_app({"app_key": expected}), expected)

    def test_login_free_six_app_scenario_contract(self) -> None:
        # lzx-note: Every new app must have launch, wait, switch, verify, and close actions.
        scenario = load_scenario(ROOT / "configs/automation/scenario_login_free_six_apps.json")
        required = {"CALENDAR", "RHYTHMBOX", "IMAGE_VIEWER", "SHOTWELL", "SYSTEM_MONITOR", "SOLITAIRE"}
        for app in required:
            actions = [item for item in scenario.actions if item.get("app_key") == app]
            self.assertTrue({"launch", "wait_window", "switch", "verify_foreground", "close"}.issubset({item["type"] for item in actions}))
    ...
    def test_runtime_app_ids_no_collision(self) -> None:
        ...
        scope = json.loads((repo_root / "lzx/service/configs/runtime/runtime_app_scope.json").read_text(encoding="utf-8"))  # lzx-note
