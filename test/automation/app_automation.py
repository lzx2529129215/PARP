...
def infer_app(action: dict[str, Any]) -> str:
    ...
    if "bilibili" in text or "哔哩哔哩" in text:
        return "BILIBILI"
    if "firefox" in text or "epiphany" in text:
        return "FIREFOX"
    ...
    if "gnome-calculator" in text or "calculator" in text or "计算器" in text:
        return "CALCULATOR"
    if "gnome-calendar" in text or "org.gnome.calendar" in text:
        return "CALENDAR"  # lzx-note
    if "rhythmbox" in text:
        return "RHYTHMBOX"  # lzx-note
    if re.search(r"(^|[| /])eog($|[| /])", text) or "image viewer" in text or "图像查看器" in text:
        return "IMAGE_VIEWER"  # lzx-note
    if "shotwell" in text:
        return "SHOTWELL"  # lzx-note
    if "gnome-system-monitor" in text or "system monitor" in text or "系统监视器" in text:
        return "SYSTEM_MONITOR"  # lzx-note
    if re.search(r"(^|[| /])(?:sol|aisleriot)($|[| /])", text):
        return "SOLITAIRE"  # lzx-note
...
def map_window_app(title: str, wm_class: str, pid_comm: str, cgroup_path: str = "") -> str:
    ...
    if "gnome-calculator" in text or "calculator" in text or "计算器" in text:
        return "CALCULATOR"
    if process_tokens.intersection({"gnome-calendar", "org.gnome.calendar"}):
        return "CALENDAR"  # lzx-note
    if "rhythmbox" in process_tokens:
        return "RHYTHMBOX"  # lzx-note
    if "eog" in process_tokens or "image viewer" in text or "图像查看器" in text:
        return "IMAGE_VIEWER"  # lzx-note
    if "shotwell" in process_tokens:
        return "SHOTWELL"  # lzx-note
    if process_tokens.intersection({"gnome-system-monitor", "gnome-system-mo"}) or "system monitor" in text or "系统监视器" in text:
        return "SYSTEM_MONITOR"  # lzx-note
    if process_tokens.intersection({"sol", "aisleriot"}) or "aisleriot" in text:
        return "SOLITAIRE"  # lzx-note
    if "keepassxc" in text or "keepass" in text:
        return "KEEPASSXC"
    if (
