...
新增 Calendar、Rhythmbox、Image Viewer（EOG）、Shotwell、System Monitor 和 AisleRiot Solitaire。场景不访问网络、不要求账号，并在本地生成确定性的 PNG/WAV 素材：

```bash
automation/run_rich_scenario.sh login-free-six --dry-run
automation/run_rich_scenario.sh login-free-six \
  --trace-output outputs/automation_login_free/automation_trace.csv
python3 automation/scripts/verify_login_free_apps.py \
  outputs/automation_login_free/automation_trace.csv \
  --output outputs/automation_login_free/verification.json
```

每个应用均执行“启动 → 内容窗口等待 → 两轮切换 → 前台校验 → 应用内按键 → cgroup 安全关闭”。生成的素材和结果位于 `outputs/automation_login_free/`。`# lzx-note`
...
automation/run_automation.sh \
  ...
  --trace-output outputs/automation/session_files_001/automation_trace.csv \
...
采集由常驻 user service 独立负责：

```bash
systemctl --user status parp-runtime-monitor.service
```

自动化只负责复现场景并在 `test/outputs` 保存 action trace：

```bash
cd /home/lzx/Desktop/PARP/test
automation/run_automation.sh \
  --scenario configs/automation/scenario_local_wps.json \
  --trace-output outputs/automation/wps_auto/automation_trace.csv
```

服务每次启动的数据独立位于：

```text
/home/lzx/Desktop/PARP/lzx/service/outputs/runtime_monitor/service_<boot-id>_<timestamp>/
```
...
需要训练数据对齐时，显式指定当次 service 目录和 automation trace：

```bash
cd /home/lzx/Desktop/PARP
python3 lzx/service/runtime_monitor/scripts/align_automation_monitor.py \
  --features lzx/service/outputs/runtime_monitor/<service-session>/model/global_state_1s.csv \
  --trace test/outputs/automation/session_files_001/automation_trace.csv \
  --output test/outputs/automation/session_files_001/features_1s.labeled.csv \
  --labels-output test/outputs/automation/session_files_001/labels.csv \
