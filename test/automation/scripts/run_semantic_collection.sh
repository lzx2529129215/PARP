...
SERVICE_ROOT="$REPO_ROOT/lzx/service" # lzx-note
RUNTIME_ROOT="$SERVICE_ROOT/runtime_monitor" # lzx-note
SCENARIO=""
SESSION_ID="semantic_$(date +%Y%m%d_%H%M%S)"
DRY_RUN=0
CALIBRATION_ONLY=0
ALLOW_EXTERNAL=0
ASSET_MANIFEST="$TEST_ROOT/automation/semantic/assets/assets_manifest.example.json"
APP_PROFILE=""
MONITOR_DURATION=90
SKIP_OPTIONAL=0
...
[[ "$SCENARIO" = /* ]] || SCENARIO="$TEST_ROOT/$SCENARIO"

SESSION_DIR="$SERVICE_ROOT/outputs/runtime_monitor/$SESSION_ID"
WORK_DIR="$TEST_ROOT/outputs/automation/semantic_collection_$SESSION_ID"
...
PROBE_ARGS=(--output-dir "$WORK_DIR/capability" --candidate-runtime-config "$WORK_DIR/capability/runtime_app_scope.candidates.json")
python3 "$TEST_ROOT/automation/scripts/probe_semantic_apps.py" "${PROBE_ARGS[@]}" >"$WORK_DIR/logs/probe.log" 2>&1
python3 "$TEST_ROOT/automation/scripts/validate_automation_assets.py" --manifest "$ASSET_MANIFEST" --output "$WORK_DIR/capability/asset_validation.csv" >"$WORK_DIR/logs/assets.log" 2>&1
...
python3 "$TEST_ROOT/automation/scripts/compile_semantic_scenario.py" "${COMPILE_ARGS[@]}" >"$WORK_DIR/logs/compile.log" 2>&1

if ((DRY_RUN)); then
  python3 "$TEST_ROOT/automation/app_automation.py" "$COMPILED" --dry-run --session-id "$SESSION_ID" --scenario-id "$(basename "${SCENARIO%.json}")" --trace-output "$WORK_DIR/automation_trace.csv" >"$WORK_DIR/logs/automation.log" 2>&1
...
python3 "$RUNTIME_ROOT/monitor.py" \
  --session-id "$SESSION_ID" --output-dir "$SERVICE_ROOT/outputs/runtime_monitor" \
  ...
  --app-scope-config "$SERVICE_ROOT/configs/runtime/runtime_app_scope.json" \
  >"$WORK_DIR/logs/monitor.log" 2>&1 &
MONITOR_PID=$!
sleep 2
AUTOMATION_ARGS=("$COMPILED" --session-id "$SESSION_ID" --scenario-id "$(basename "${SCENARIO%.json}")" --trace-output "$WORK_DIR/automation_trace.csv" --test-slice huawei-test.slice)
((CALIBRATION_ONLY)) && AUTOMATION_ARGS+=(--calibration-only --calibration-output-dir "$WORK_DIR/calibration")
set +e
python3 "$TEST_ROOT/automation/app_automation.py" "${AUTOMATION_ARGS[@]}" >"$WORK_DIR/logs/automation.log" 2>&1
AUTOMATION_RC=$?
set -e
wait "$MONITOR_PID" || MONITOR_RC=$?
MONITOR_RC=${MONITOR_RC:-0}

python3 "$RUNTIME_ROOT/scripts/align_semantic_operations.py" \
  --trace "$WORK_DIR/automation_trace.csv" --foreground "$SESSION_DIR/model/global_state_1s.csv" \
...
python3 "$RUNTIME_ROOT/scripts/build_semantic_operation_summary.py" --alignment "$WORK_DIR/alignment/semantic_operation_alignment.csv" --output-dir "$WORK_DIR/reports" || true
