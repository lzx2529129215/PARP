...
LSTM_GROUP_VOCAB="${LSTM_GROUP_VOCAB:-$PRED/data/vocab/test1/user_group_vocab.json}" # lzx-note
RUNTIME_TARGET_APPS="${RUNTIME_TARGET_APPS:-FIREFOX,LIBREOFFICE,VLC,GIMP,AUDACITY,THUNDERBIRD,TELEGRAM,EVINCE,FILES,CALCULATOR}" # lzx-note
PARP_MODEL_NAME="${PARP_MODEL_NAME:-AppLSTM-v3-test1}" # lzx-note
PARP_MODEL_VERSION="${PARP_MODEL_VERSION:-401}" # lzx-note
...
NESTED_X_SERVER="${NESTED_X_SERVER:-xephyr}" # lzx-note
HOST_DISPLAY="${HOST_DISPLAY:-:0}"
HOST_XAUTHORITY="${HOST_XAUTHORITY:-$(ls -t /run/user/$(id -u)/.mutter-Xwaylandauth.* 2>/dev/null | head -1 || true)}"
SCENARIO_PATH="${SCENARIO_PATH:-$PARP_AUTOMATION_CONFIG_ROOT/scenario_test1_app_switch.json}"
...
if [ "$NESTED_X_SERVER" = "xephyr" ] && [ -z "$HOST_XAUTHORITY" ]; then # lzx-note
  if DISPLAY="$HOST_DISPLAY" XAUTHORITY=/dev/null xdpyinfo >/dev/null 2>&1; then # lzx-note
    HOST_XAUTHORITY=/dev/null # lzx-note
  else # lzx-note
    echo "无法访问宿主 X11 display，且未找到 GNOME Xwayland XAUTHORITY" >&2 # lzx-note
    exit 1 # lzx-note
  fi # lzx-note
fi # lzx-note
...

{
  ...
  echo "lstm_checkpoint=$LSTM_CHECKPOINT" # lzx-note
  echo "lstm_app_vocab=$LSTM_APP_VOCAB" # lzx-note
  echo "lstm_group_vocab=$LSTM_GROUP_VOCAB" # lzx-note
  echo "runtime_target_apps=$RUNTIME_TARGET_APPS" # lzx-note
...
case "$NESTED_X_SERVER" in # lzx-note
  xephyr) # lzx-note
    systemd-run --user --unit="$XEPHYR_UNIT" --collect \
      --setenv=DISPLAY="$HOST_DISPLAY" --setenv=XAUTHORITY="$HOST_XAUTHORITY" \
      Xephyr "$NESTED_DISPLAY" -screen "$NESTED_SCREEN" -ac -br -noreset # lzx-note
    ;; # lzx-note
  xvfb) # lzx-note
    systemd-run --user --unit="$XEPHYR_UNIT" --collect \
      Xvfb "$NESTED_DISPLAY" -screen 0 "${NESTED_SCREEN}x24" -ac -noreset # lzx-note
    ;; # lzx-note
  *) echo "NESTED_X_SERVER must be xephyr or xvfb" >&2; exit 2 ;; # lzx-note
esac # lzx-note
...
MONITOR_COMMAND=(
  ...
  --group-vocab "$LSTM_GROUP_VOCAB" # lzx-note
  ...
  --parp-model-name "$PARP_MODEL_NAME" --parp-model-version "$PARP_MODEL_VERSION" # lzx-note
  --parp-schema-version parp-app-prior-v1 --parp-prior-ttl-ms 180000
  --target-apps "$RUNTIME_TARGET_APPS" # lzx-note
...
sleep 3
set +e
bash "$PARP_AUTOMATION_ROOT/run_automation.sh" \
