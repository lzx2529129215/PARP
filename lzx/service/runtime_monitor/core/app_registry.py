...
class AppRegistry:
    def __init__(
        ...
        # lzx-note: A resident service must never repeat its complete lifetime
        # history in every one-second row.  A fixed window preserves short-term
        # model context while keeping both memory and each CSV row bounded.
        self.history_window = max(0, history_window)
        self.app_history: deque[str] = deque(maxlen=self.history_window or None)
        self.duration_history: deque[str] = deque(maxlen=self.history_window or None)
    ...
    def update(self, samples: list[ProcessSample], foreground: ForegroundState) -> None:
        ...
        if self.history_window and foreground_app and foreground_app != self._last_foreground_app:
