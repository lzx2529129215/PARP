...
#define PARP_TIER_STATE_TRIES		64
...
struct parp_tier_stats {
	...
	atomic64_t state_lock_failures;
...
static spinlock_t parp_tier_state_locks[PARP_TIER_STATE_LOCKS] = {
	[0 ... (PARP_TIER_STATE_LOCKS - 1)] =
		__SPIN_LOCK_UNLOCKED(parp_tier_state_locks)
}; /* #lzx */
...
static spinlock_t *parp_state_lock(struct parp_reuse_page_ext *ext)
{
	return &parp_tier_state_locks[
		hash_ptr(ext, PARP_TIER_STATE_LOCK_BITS)];
} /* #lzx */

static bool parp_state_write_begin(struct parp_reuse_page_ext *ext,
		u32 *stable_state, unsigned long *irq_flags)
{
	spinlock_t *lock = parp_state_lock(ext);
	u32 old;
	u32 seq;
	u32 locked;

	spin_lock_irqsave(lock, *irq_flags);
	old = READ_ONCE(ext->state);
	seq = parp_state_seq(old);
	if (WARN_ON_ONCE(seq & 1)) {
		spin_unlock_irqrestore(lock, *irq_flags);
		atomic64_inc(&parp_tier_stats.state_lock_failures);
		return false;
	}
	locked = (old & ~PARP_STATE_SEQ_MASK) |
		FIELD_PREP(PARP_STATE_SEQ_MASK,
			   (seq + 1) & FIELD_MAX(PARP_STATE_SEQ_MASK));
	/* Expose an odd sequence before changing the lockless-read payload. */
	smp_store_release(&ext->state, locked);
	*stable_state = old;
	return true;
}

static void parp_state_write_end(struct parp_reuse_page_ext *ext,
		u32 old_state, u32 new_state, unsigned long irq_flags)
{
	spinlock_t *lock = parp_state_lock(ext);
		...
		enum parp_access_event event)
{
	...
	if (!ext)
		return;
	if (!parp_state_write_begin(ext, &old_state, &irq_flags)) {
	...
	parp_state_write_end(ext, old_state, new_state, irq_flags);
emit:
		...
		u16 *claimed_sequence, enum parp_tier_bypass_reason *bypass)
{
	...
	if (expected_sequence >= 0 &&
		...
		*bypass = PARP_TIER_BYPASS_GENERATION_RACE;
		parp_state_write_end(ext, old_state, old_state, irq_flags);
		page_ext_put(page_ext);
		return false;
	}
	epochs = READ_ONCE(ext->decision_epochs);
...
{
	return atomic_read(&parp_tier_state_fault);
} /* #lzx */
...
ssize_t parp_effective_tier_format_stats(char *buf, size_t size)
{
	...
	len = scnprintf(buf, size,
		...
		atomic64_read(&parp_tier_stats.state_lock_failures),
