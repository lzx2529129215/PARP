...
2. 当前完整 APPLY 没有达到交付指标。相对 Native，PageFault 只下降 `0.04%`，OOM kill 和失败峰值均为 `0%` 改善；虽然 direct scan 下降 `67.49%`、PSI stall 和启动代理延迟下降，但总 pgscan 增加 `42.35%`、major fault 增加 `11.12%`、memcg reclaim 总耗时增加 `105.83%`。

因此，本报告可以证明 reclaim-bin 代码路径真实生效以及配对实验可执行，但不能证明当前完整 APPLY 已经优化 PageFault 或 OOM。
...
| 组别 | 内核 | effective-tier | Tier2 | reclaim-bin |
|---|---|---:|---:|---:|
| A Native | `6.17.13-native-6.17.13` | 0 | 0 | 0 |
| B Full APPLY | `6.17.13-parp-lzx-v4.2-apply-reclaim-bin #10` | 1 | 1 | 1 |
| C Effective only | 同 B | 1 | 0 | 0（路径不启用） |
| D Tier2 + reclaim-bin | 同 B | 0 | 1 | 1 |
| E Full no-reclaim | `6.17.13-parp-lzx-v4.2-apply-no-reclaim-bin #11` | 1 | 1 | 0（编译移除） |
...
这说明 reclaim-bin 在当前诊断场景中减少了扫描、refault、stall 和回收耗时，但没有降低 OOM kill，也没有给 PageFault 带来有意义的改善。

## 组合交互

C 与 D 单独启用时的多个回收指标优于 B 完整组合。尤其 B 相比 D：
...
这表明当前 effective-tier 与 Tier2/reclaim-bin 组合存在明显的负交互，或者至少对种子 `20260821` 的高压力状态非常敏感。现阶段不应把单模块的诊断收益直接外推为完整组合收益。
...
1. effective-tier 的模型来源仍为 `ENGINEERING_FIXTURE_UNTRAINED`，所有纳入统计轮次的 `scores=0`，因此不能把本次数据解释为训练后页面预测模型的收益。
...
4. 每组只有三个有效轮次，且 `20260821` 在多组中明显比另外两个 seed 更重。应扩大轮数并报告置信区间。
5. 需要完成新的 LSAPP 对齐训练集和 checkpoint，并让在线服务把应用预测上下文写入内核，再重新测试 Full APPLY。
